...
def load_thread_tree(
    thread_id: str,
) -> tuple[str, dict[str, dict], dict[str, list[str]]]:
    ...
    sibling_threads = list(mongo.threads().find({"root_thread_id": root_id}))
    if not any(st["_id"] == root_id for st in sibling_threads):
        root_doc = mongo.threads().find_one({"_id": root_id})
        if root_doc:
            sibling_threads.append(root_doc)
